...
#define BOOST_GEOMETRY_DETAIL_SPECIALIZE_SEGMENT_ACCESS(Segment, Point,        \
                                                        Index0, Index1)        \
  template <size_t D> struct indexed_access<Segment, min_corner, D> {          \
  ...
  template <size_t D> struct indexed_access<Segment, max_corner, D> {          \
    typedef typename coordinate_type<Point>::type ct;                          \
    static inline ct get(Segment const &b) {                                   \
...
#define BOOST_GEOMETRY_DETAIL_SPECIALIZE_SEGMENT_ACCESS_TEMPLATIZED(           \
    Segment, Index0, Index1)                                                   \
...
#define BOOST_GEOMETRY_DETAIL_SPECIALIZE_SEGMENT_ACCESS_4VALUES(               \
    Segment, Point, Left, Bottom, Right, Top)                                  \
...
#define BOOST_GEOMETRY_REGISTER_SEGMENT(Segment, PointType, Index0, Index1)    \
  ...
  BOOST_GEOMETRY_DETAIL_SPECIALIZE_SEGMENT_ACCESS(Segment, PointType, Index0,  \
                                                  Index1)                      \
  }                                                                            \
  }                                                                            \
  }
...
#define BOOST_GEOMETRY_REGISTER_SEGMENT_TEMPLATIZED(Segment, Index0, Index1)   \
  ...
  BOOST_GEOMETRY_DETAIL_SPECIALIZE_SEGMENT_ACCESS_TEMPLATIZED(Segment, Index0, \
                                                              Index1)          \
  }                                                                            \
  }                                                                            \
  }

#define BOOST_GEOMETRY_REGISTER_SEGMENT_2D_4VALUES(Segment, PointType, Left,   \
                                                   Bottom, Right, Top)         \
  namespace boost {                                                            \
  namespace geometry {                                                         \
  namespace traits {                                                           \
  BOOST_GEOMETRY_DETAIL_SPECIALIZE_SEGMENT_TRAITS(Segment, PointType)          \
  BOOST_GEOMETRY_DETAIL_SPECIALIZE_SEGMENT_ACCESS_4VALUES(Segment, PointType,  \
                                                          Left, Bottom, Right, \
                                                          Top)                 \
  }                                                                            \
  }                                                                            \
