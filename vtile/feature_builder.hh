...
template <typename CoordinateType> struct point_handler {
  using geom_type = mapbox::geometry::multi_point<CoordinateType>;
  point_handler(geom_type &geom, std::uint32_t dx, std::uint32_t dy,
                std::uint32_t zoom_factor,
                mapbox::geometry::box<CoordinateType> const &bbox)
      : geom_(geom), bbox_(bbox), dx_(dx), dy_(dy), zoom_factor_(zoom_factor) {}

  void points_begin(std::uint32_t /*unused*/) {}

  void points_point(vtzero::point const &pt) {
    CoordinateType x = pt.x * static_cast<std::int32_t>(zoom_factor_) -
                       static_cast<std::int32_t>(dx_);
    CoordinateType y = pt.y * static_cast<std::int32_t>(zoom_factor_) -
                       static_cast<std::int32_t>(dy_);
...
template <typename CoordinateType> struct line_string_handler {
  using geom_type = mapbox::geometry::multi_line_string<CoordinateType>;

  line_string_handler(geom_type &geom, std::uint32_t dx, std::uint32_t dy,
                      std::uint32_t zoom_factor)
      : geom_(geom), dx_(dx), dy_(dy), zoom_factor_(zoom_factor) {}
...
using annotated_ring =
    std::pair<mapbox::geometry::linear_ring<CoordinateType>, vtzero::ring_type>;

template <typename CoordinateType> struct polygon_handler {
  using geom_type = std::vector<annotated_ring<CoordinateType>>;
  polygon_handler(geom_type &geom, std::uint32_t dx, std::uint32_t dy,
                  std::uint32_t zoom_factor)
      : geom_(geom), dx_(dx), dy_(dy), zoom_factor_(zoom_factor) {}
  ...
  void ring_end(vtzero::ring_type type) { geom_.back().second = type; }
...
template <typename CoordinateType> struct overzoomed_feature_builder {
  ...
  overzoomed_feature_builder(vtzero::layer_builder &layer_builder,
                             ...
                             std::uint32_t zoom_factor)
      : layer_builder_{layer_builder}, mapper_{mapper}, bbox_{bbox}, dx_{dx},
        dy_{dy}, zoom_factor_{zoom_factor} {}
  ...
  void apply_geometry_point(vtzero::feature const &feature) {
    ...
    vtzero::decode_point_geometry(
        feature.geometry(), detail::point_handler<coordinate_type>(
                                multi_point, dx_, dy_, zoom_factor_, bbox_));
  ...
  void apply_geometry_linestring(vtzero::feature const &feature) {
    ...
    vtzero::decode_linestring_geometry(
        feature.geometry(), detail::line_string_handler<coordinate_type>(
                                multi_line, dx_, dy_, zoom_factor_));
    ...
    if (!result.empty()) {
      ...
      for (auto const &l : result) {
        if (l.size() > 1) {
          ...
          auto itr = l.cbegin();
          auto last_pt = *itr++;
          feature_builder.set_point(static_cast<int>(last_pt.x),
                                    static_cast<int>(last_pt.y));
          for (auto const &end = l.end(); itr != end; ++itr) {
            if (*itr != last_pt) {
              valid = true;
              feature_builder.set_point(static_cast<int>(itr->x),
  ...
  void apply_geometry_polygon(vtzero::feature const &feature) {
    ...
    vtzero::decode_polygon_geometry(
        feature.geometry(),
        detail::polygon_handler<CoordinateType>(rings, dx_, dy_, zoom_factor_));
