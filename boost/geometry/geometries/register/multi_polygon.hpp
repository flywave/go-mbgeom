...
\ingroup register
\details \details_macro{BOOST_GEOMETRY_REGISTER_MULTI_POLYGON, multi_polygon}
The multi_polygon may contain template parameters, which must be specified then.
...
\ingroup register
\details \details_macro{BOOST_GEOMETRY_REGISTER_MULTI_POLYGON_TEMPLATED,
templated multi_polygon} \details_macro_templated{multi_polygon, polygon} \param
MultiPolygon \param_macro_type{multi_polygon (without template parameters)}
...
#define BOOST_GEOMETRY_REGISTER_MULTI_POLYGON_TEMPLATED(MultiPolygon)          \
  ...
  namespace geometry {                                                         \
  namespace traits {                                                           \
  template <typename Polygon> struct tag<MultiPolygon<Polygon>> {              \
