...
\ingroup register
\details \details_macro{BOOST_GEOMETRY_REGISTER_MULTI_POINT_TEMPLATED, templated
multi_point} \details_macro_templated{multi_point, point} \param MultiPoint
\param_macro_type{multi_point (without template parameters)}
...
#define BOOST_GEOMETRY_REGISTER_MULTI_POINT_TEMPLATED(MultiPoint)              \
  ...
  namespace geometry {                                                         \
  namespace traits {                                                           \
  template <typename Point> struct tag<MultiPoint<Point>> {                    \
