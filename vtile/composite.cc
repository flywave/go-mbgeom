...
struct TileObject {
  TileObject(std::uint32_t z0, std::uint32_t x0, std::uint32_t y0,
             Napi::Buffer<char> const &buffer, std::vector<std::string> layers0)
      : z{z0}, x{x0}, y{y0}, data{buffer.Data(), buffer.Length()},
        buffer_ref{Napi::Persistent(buffer)}, layers{std::move(layers0)} {}
...
struct BatonType {
  explicit BatonType(std::size_t num_tiles) { tiles.reserve(num_tiles); }
    ...
    try {
      ...
      for (auto const &tile_obj : baton_data_->tiles) {
        if (vtile::within_target(*tile_obj, target_z, target_x, target_y)) {
          ...
          if (gzip::is_compressed(tile_obj->data.data(),
                                  tile_obj->data.size())) {
            buffer_cache.push_back(std::make_unique<std::vector<char>>());
            gzip::Decompressor decompressor;
            decompressor.decompress(*buffer_cache.back(), tile_obj->data.data(),
                                    tile_obj->data.size());
            tile_view = protozero::data_view{buffer_cache.back()->data(),
                                             buffer_cache.back()->size()};
          } else {
          ...
          while (auto layer = tile.next_layer()) {
            ...
            if (std::find(std::begin(names), std::end(names), name) ==
              ...
              // if include_layers is not empty, keep layer if we can find its
              // name in the vector
  ...
  std::vector<napi_value> GetResult(Napi::Env env) override {
    if (output_buffer_) {
      ...
      auto buffer = Napi::Buffer<char>::New(
          env, tile_buffer.empty() ? nullptr : &tile_buffer[0],
          tile_buffer.size(),
      ...
      Napi::MemoryManagement::AdjustExternalMemory(
          env, static_cast<std::int64_t>(tile_buffer.size()));
...
Napi::Value composite(Napi::CallbackInfo const &info) {
  ...
  if (length == 0) {
    Napi::Error::New(info.Env(), "last argument must be a callback function")
  ...
  if (!info[0].IsArray()) {
    return utils::CallbackError(
        "first arg 'tiles' must be an array of tile objects", info);
  }
  ...
  if (num_tiles <= 0) {
    return utils::CallbackError(
        "'tiles' array must be of length greater than 0", info);
  }

  std::unique_ptr<BatonType> baton_data =
      std::make_unique<BatonType>(num_tiles);

  for (std::uint32_t t = 0; t < num_tiles; ++t) {
    Napi::Value tile_val = tiles.Get(t);
    if (!tile_val.IsObject()) {
      return utils::CallbackError("items in 'tiles' array must be objects",
                                  info);
    }
    ...
    if (!tile_obj.Has(Napi::String::New(info.Env(), "buffer"))) {
      return utils::CallbackError(
          "item in 'tiles' array does not include a buffer value", info);
    }
    Napi::Value buf_val = tile_obj.Get(Napi::String::New(info.Env(), "buffer"));
    if (buf_val.IsNull() || buf_val.IsUndefined()) {
      return utils::CallbackError(
          "buffer value in 'tiles' array item is null or undefined", info);
    }

    Napi::Object buffer_obj = buf_val.As<Napi::Object>();
    if (!buffer_obj.IsBuffer()) {
      return utils::CallbackError(
          "buffer value in 'tiles' array item is not a true buffer", info);
    }
    ...
    if (!tile_obj.Has(Napi::String::New(info.Env(), "z"))) {
      return utils::CallbackError(
          "item in 'tiles' array does not include a 'z' value", info);
    }
    Napi::Value z_val = tile_obj.Get(Napi::String::New(info.Env(), "z"));
    if (!z_val.IsNumber()) {
      return utils::CallbackError(
          "'z' value in 'tiles' array item is not an int32", info);
    }
    ...
    if (!tile_obj.Has(Napi::String::New(info.Env(), "x"))) {
      return utils::CallbackError(
          "item in 'tiles' array does not include a 'x' value", info);
    }
    Napi::Value x_val = tile_obj.Get(Napi::String::New(info.Env(), "x"));
    if (!x_val.IsNumber()) {
      return utils::CallbackError(
          "'x' value in 'tiles' array item is not an int32", info);
    }
    ...
    if (!tile_obj.Has(Napi::String::New(info.Env(), "y"))) {
      return utils::CallbackError(
          "item in 'tiles' array does not include a 'y' value", info);
    }
    Napi::Value y_val = tile_obj.Get(Napi::String::New(info.Env(), "y"));
    if (!y_val.IsNumber()) {
      return utils::CallbackError(
          "'y' value in 'tiles' array item is not an int32", info);
    }
    ...
    if (tile_obj.Has(Napi::String::New(info.Env(), "layers"))) {
      Napi::Value layers_val =
          tile_obj.Get(Napi::String::New(info.Env(), "layers"));

      // is the layers property an array?
      if (!layers_val.IsArray()) {
        return utils::CallbackError(
            "'layers' value in the 'tiles' array must be an array", info);
      }
      ...
      if (num_layers == 0) {
        return utils::CallbackError(
            "'layers' array must be of length greater than 0", info);
      }
      ...
      for (std::uint32_t l = 0; l < num_layers; ++l) {
        ...
        if (!layer_val.IsString()) {
          return utils::CallbackError("items in 'layers' array must be strings",
                                      info);
        }
    ...
    baton_data->tiles.push_back(
        std::make_unique<TileObject>(z, x, y, buffer, layers));
  }
  ...
  Napi::Value z_val_maprequest =
      zxy_maprequest.Get(Napi::String::New(info.Env(), "z"));
  ...
  Napi::Value x_val_maprequest =
      zxy_maprequest.Get(Napi::String::New(info.Env(), "x"));
  ...
  Napi::Value y_val_maprequest =
      zxy_maprequest.Get(Napi::String::New(info.Env(), "y"));
  ...
  if (info.Length() > 3) // options
  {
    ...
    if (options.Has(Napi::String::New(info.Env(), "buffer_size"))) {
      Napi::Value bs_value =
          options.Get(Napi::String::New(info.Env(), "buffer_size"));
      ...
      if (buffer_size < 0) {
        return utils::CallbackError("'buffer_size' must be a positive int32",
                                    info);
      }
      baton_data->buffer_size = buffer_size;
    }
    if (options.Has(Napi::String::New(info.Env(), "compress"))) {
      Napi::Value comp_value =
          options.Get(Napi::String::New(info.Env(), "compress"));
