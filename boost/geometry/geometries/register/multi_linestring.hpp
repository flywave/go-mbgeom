...
\ingroup register
\details \details_macro{BOOST_GEOMETRY_REGISTER_MULTI_LINESTRING,
multi_linestring} The multi_linestring may contain template parameters, which
must be specified then. \param MultiLineString
\param_macro_type{multi_linestring}
...
#define BOOST_GEOMETRY_REGISTER_MULTI_LINESTRING(MultiLineString)              \
  ...
  namespace geometry {                                                         \
  namespace traits {                                                           \
  template <> struct tag<MultiLineString> {                                    \
...
\ingroup register
\details \details_macro{BOOST_GEOMETRY_REGISTER_MULTI_LINESTRING_TEMPLATED,
templated multi_linestring} \details_macro_templated{multi_linestring,
linestring} \param MultiLineString \param_macro_type{multi_linestring (without
template parameters)}
...
#define BOOST_GEOMETRY_REGISTER_MULTI_LINESTRING_TEMPLATED(MultiLineString)    \
  ...
  namespace geometry {                                                         \
  namespace traits {                                                           \
  template <typename LineString> struct tag<MultiLineString<LineString>> {     \
    typedef multi_linestring_tag type;                                         \
  };                                                                           \
  }                                                                            \
  }                                                                            \
