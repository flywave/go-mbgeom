...
\ingroup register
\details \details_macro{BOOST_GEOMETRY_REGISTER_LINESTRING_TEMPLATED, templated
linestring} \details_macro_templated{linestring, point} \param Linestring
\param_macro_type{linestring (without template parameters)}
...
#define BOOST_GEOMETRY_REGISTER_LINESTRING_TEMPLATED(Linestring)               \
  ...
  namespace geometry {                                                         \
  namespace traits {                                                           \
  template <typename P> struct tag<Linestring<P>> {                            \
